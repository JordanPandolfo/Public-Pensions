...
module globals
    ...
    real*8, parameter :: sigma_eps_pub    = 0.0169d0
...
!
    ...
    real*8, parameter :: phi_g = 1d0                 ! share of pension surplus rebated to all workers
    ...
    ! reform parameters
    real*8 :: closed_dc_rate = 0.05d0
    real*8, dimension(JR)    :: dc_con
    real*8, parameter        :: hybrid_dc_rate = 0.05d0
    real*8, dimension(JR)    :: ben_red
    real*8, dimension(JR,JJ) :: pen_pub
    real*8 :: net_cola_loss                ! loss in real value of pension due to inflation
        ...
        if(ij_com+1 >= JR)then     ! if retired
            do isr = 1, NSR
                ...
                if ( ij_com+1 == JR ) then  ! first year of retirement
                    ...
                    port_return =  1d0 + r_f + port_share*( mu_r + vtheta_pub(isr) )  ! state pension return
        ...
        do tt=1,JR  !for current/future year, today
            ...
            do ii = 0,chi_n  !for each post-contribution funded status, today
                ...
                do jj = 1,NSR  !for each market return, tomorrow
                    ...
                    port_return = 1d0 + r_f + port_share*( mu_r + vtheta_pub(jj) )
